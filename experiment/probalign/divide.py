...
#First alignment using probalign, outputing to temp folder
files = os.listdir(originalfilepath)
for _filename in files:
    os.system("cd " + originalfilepath + " && " + "probalign "  + _filename + ">>" + temp_path + _filename + "_aligned")
############################################################################################################################
...
for _filename in files:
    ...
    protein = []
    tempstring = []
    #Reading each file, deal with probalign' line feed strategy.
    ...
    with open(_out_file_path + "in_pt0/"+originalfname+"_pt0", 'w') as file:
        ...
        for i in range(len(original_protein_n)):
            ...
            if line == '\n':
                ...
                nametemp = name
                linetemp = line
    #If there is only one sequence in alignment file, probalign do not recogonize the file as an illegal input.
    #Generate aligned file without using probalign
    if empty_flag_0.count(0) == 1:
        with open(pt0_outpath + originalfname + "_pt0_aligned", "w") as file:
            file.write(nametemp)
            file.write(linetemp)
    else:
        #Generate aligned file using probalign
        os.system("probalign "  + _out_file_path + "in_pt0/" + originalfname + "_pt0 >> " + pt0_outpath + originalfname + "_pt0_aligned ")
    ...
    if empty_flag_1.count(0) == 1:
        ...
        #Generate aligned file using probalign
        os.system("probalign "  + _out_file_path + "in_pt1/" + originalfname + "_pt1 >> " + pt1_outpath + originalfname + "_pt1_aligned ")
    ...
    if empty_flag_2.count(0) == 1:
        ...
        #Generate aligned file using probalign
        os.system("probalign "  + _out_file_path + "in_pt2/" + originalfname + "_pt2 >> " + pt2_outpath + originalfname + "_pt2_aligned ")
